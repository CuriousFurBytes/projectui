...
function buildTextualRouting(project: ProjectState): string {
  const layers = project.layers ?? [];
  const steps = project.timelineSteps ?? [];
  const transitions = project.timelineTransitions ?? [];
  if (layers.length <= 1 || transitions.length === 0) return '';
  ...
  const screenName = (layerId: string) => {
    ...
    return (l?.name ?? layerId).replace(/[^a-zA-Z0-9_]/g, '_');
  };

  const keyCases = transitions.map((t) => {
    ...
    const trigger = t.trigger ?? 'enter';
    return `        if self.current_screen == ${pyStr(screenName(fromLayerId))} and key == ${pyStr(trigger)}:\n            self.current_screen = ${pyStr(screenName(toLayerId))}\n            self.refresh()`;
  }).filter(Boolean).join('\n');

  if (!keyCases) return '';

  return `\n    current_screen: str = ${pyStr(screenName(layers[0]?.id ?? ''))}\n\n    def on_key(self, event) -> None:\n        key = event.key\n${keyCases}\n`;
}
...
export function exportTextual(project: ProjectState): string {
  ...
  const routing = buildTextualRouting(project);

  return `"""
Auto-generated by ProjecTUI.
